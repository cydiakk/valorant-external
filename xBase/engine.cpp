...
namespace engine {
	...
	{
		uintptr_t bonearray = core::read<uintptr_t>(globals::target_proc::target_process_id, mesh + 0x518);

		if (!utils::is_valid_addr(bonearray))
			bonearray = core::read<uintptr_t>(globals::target_proc::target_process_id, mesh + 0x528);
